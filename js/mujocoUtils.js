...
export function setupGUI(parentContext) {
  ...
  parentContext.gui
    .add(parentContext.params, "scene", {
      Starfish: "output_starfish.xml",
      Snake: "output_snake.xml",
      HotDog: "hotdog.xml",
    })
    .name("Example Scene")
    .onChange(reload);
...
export async function loadSceneFromURL(mujoco, filename, parent) {
  ...
  console.log("qpos: ", parent.simulation.qpos.length);
  ...
  // Parse lights.
  console.log("nlight: ", model.light_directional);
...
export async function downloadExampleScenesFolder(mujoco) {
  ...
  let requests = allFiles.map((url) =>
    fetch("https://msan030.github.io/xenojoco_static/scenes/" + url)
  );
