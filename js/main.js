...
import { multiply, add } from "https://cdn.jsdelivr.net/npm/mathjs@13.2.0/+esm";

// Load the MuJoCo Module
const mujoco = await load_mujoco();

// Set up Emscripten's Virtual File System
var initialScene = "output_starfish.xml";
...
mujoco.FS.writeFile(
  ...
  await (
    await fetch(
      "https://msan030.github.io/xenojoco_static/scenes/" + initialScene
    )
  ).text()
);
...
export class MuJoCoDemo {
  constructor() {
    ...
    console.log("qpos: ", this.simulation.qpos.length);
    ...
    const mujocoContainer = document.getElementById("mujoco");
    mujocoContainer.appendChild(this.container);
  ...
  async init() {
    ...
    document.getElementById("mujoco-controls").appendChild(this.gui.domElement);
  ...
  // Access and reshape xmat for a specific bodyIndex
  getRotationMatrix(data, bodyIndex) {
    const startIndex = bodyIndex * 9; // Starting point for this body's xmat
    const xmat = data.xmat; // Flat Float64Array

    // Extract 9 elements and reshape to 3x3 matrix
    return [
      [xmat[startIndex], xmat[startIndex + 1], xmat[startIndex + 2]],
      [xmat[startIndex + 3], xmat[startIndex + 4], xmat[startIndex + 5]],
      [xmat[startIndex + 6], xmat[startIndex + 7], xmat[startIndex + 8]],
    ];
  }

  local2global(bodyIndex, forceLocal, pointLocal) {
    // 3x3 rotation matrix
    // console.log("xmat", this.simulation.xmat);
    const rotationMatrix = this.getRotationMatrix(this.simulation, bodyIndex);

    // Body's position in the global frame
    const positionGlobal = this.simulation.xpos[bodyIndex];
    console.log("Position:", positionGlobal);
    console.log("Rotation Matrix:", rotationMatrix);

    // Transform force and point to the global frame
    const forceGlobal = multiply(rotationMatrix, forceLocal);
    const pointOffset = multiply(rotationMatrix, pointLocal);
    const pointGlobal = add(positionGlobal, pointOffset);

    console.log("Applying force to body", bodyIndex);
    console.log("Local Force:", forceLocal);
    console.log("Local Point:", pointLocal);
    console.log("Force:", forceGlobal);
    console.log("Point:", pointGlobal);
    return { forceGlobal, pointGlobal };
  }
    ...
    if (!this.params["paused"]) {
      ...
      while (this.mujoco_time < timeMS) {
        ...
        if (dragged && dragged.bodyID) {
          ...
          // test
          // let res = this.local2global(bodyID, [1, 2, 3], [4, 5, 6]);

          // TODO: Apply pose perturbations (mocap bodies only).
        }
        // this.simulation.applyForce(200, 1, 0, 0, 0, 0, 0, 0, 0, 2);
